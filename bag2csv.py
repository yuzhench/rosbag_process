...
for bagFile in listOfBagFiles:
	...
	for idx, (subtopic, msg, t) in tqdm(enumerate(bag.read_messages('/rgb/image_raw'))):	# for each instant in time that has data for topicName
		#parse data from this instant, which is of the form of multiple lines of "Name: value\n"
		#	- put it in the form of a list of 2-element lists
		try:
			cv_rgb = bridge.imgmsg_to_cv2(msg, "bgr8")
			# cv_rgb = np.frombuffer(msg.data, dtype=np.uint8)
			print(f"the shape is: {cv_rgb.shape}")
		except CvBridgeError as e:
			print(e)
		cv2.imwrite(rgb_folder+'/rgb_'+str(idx)+'.jpg', cv_rgb)
		 

	for idx, (subtopic, msg, t) in tqdm(enumerate(bag.read_messages('/depth_to_rgb/image_raw'))):	# for each instant in time that has data for topicName
		#parse data from this instant, which is of the form of multiple lines of "Name: value\n"
		#	- put it in the form of a list of 2-element lists
		try:
			cv_depth = bridge.imgmsg_to_cv2(msg, "16UC1")
			# cv_depth = np.frombuffer(msg.data, dtype=np.uint16)
			print(f"the shape is: {cv_depth.shape}")
		except CvBridgeError as e:
			print(e)
		np.save(depth_folder+f'/{str(idx)}_depth', cv_depth)
